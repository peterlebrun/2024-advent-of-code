...
def explore_neighbors(node, edges, subgraph=set(), depth=0):
    for n in edges[node]:
        if n in subgraph: continue

        if subgraph.issubset(edges[n]):
            subgraph.add(n)
            explore_neighbors(n, edges, subgraph.copy(), depth+1)

    return subgraph

subgraphs = set()
for c1 in edges:
    subgraphs.add(tuple(explore_neighbors(c1, edges, set([c1]))))

for x in sorted([*subgraphs], key=len, reverse=True):
    print(",".join(sorted([*x])))
    break
